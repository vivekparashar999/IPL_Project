"""
Live cricket data scraper using Cricbuzz public pages.
No API key required.
"""
import requests
import re
import json
from bs4 import BeautifulSoup

HEADERS = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 '
                  '(KHTML, like Gecko) Chrome/121.0.0.0 Safari/537.36',
    'Accept-Language': 'en-US,en;q=0.9',
}
BASE = 'https://www.cricbuzz.com'

TEAM_ABBR = {
    'LSG': 'Lucknow Super Giants', 'DC': 'Delhi Capitals',
    'MI': 'Mumbai Indians', 'CSK': 'Chennai Super Kings',
    'KKR': 'Kolkata Knight Riders', 'RCB': 'Royal Challengers Bengaluru',
    'RR': 'Rajasthan Royals', 'PBKS': 'Punjab Kings',
    'SRH': 'Sunrisers Hyderabad', 'GT': 'Gujarat Titans',
}
ABBR_TEAM = {v: k for k, v in TEAM_ABBR.items()}

VENUE_CITY = {
    'lucknow': 'Lucknow', 'delhi': 'Delhi', 'mumbai': 'Mumbai',
    'chennai': 'Chennai', 'kolkata': 'Kolkata', 'bengaluru': 'Bengaluru',
    'bangalore': 'Bengaluru', 'hyderabad': 'Hyderabad', 'ahmedabad': 'Ahmedabad',
    'jaipur': 'Jaipur', 'pune': 'Pune', 'navi mumbai': 'Navi Mumbai',
    'mohali': 'Mohali', 'dharamsala': 'Dharamsala', 'dubai': 'Dubai',
    'sharjah': 'Sharjah', 'indore': 'Indore', 'ranchi': 'Ranchi',
}


def get_live_ipl_matches():
    try:
        r = requests.get(f'{BASE}/cricket-match/live-scores', headers=HEADERS, timeout=12)
        soup = BeautifulSoup(r.text, 'lxml')
        matches = []
        seen = set()
        for link in soup.find_all('a', href=True):
            href = link.get('href', '')
            if 'premier-league-2026' in href and 'live-cricket-scores' in href:
                m = re.search(r'/live-cricket-scores/(\d+)/', href)
                if m:
                    mid = m.group(1)
                    if mid in seen: continue
                    seen.add(mid)
                    name = link.text.strip()
                    if name and len(name) > 5:
                        matches.append({'id': mid, 'name': name, 'status': ''})
        return matches
    except Exception as e:
        print(f'get_live_ipl_matches error: {e}')
        return []


def get_live_match_data(match_id):
    try:
        url = f'{BASE}/live-cricket-scores/{match_id}'
        r = requests.get(url, headers=HEADERS, timeout=12)
        text = r.text
        return _parse_match_page(match_id, text)
    except Exception as e:
        print(f'get_live_match_data error: {e}')
        return None


def _parse_match_page(match_id, text):
    # ── 1. Title & match name ──────────────────────────────────────────────────
    title_m = re.search(r'<title>(.*?)</title>', text)
    raw_title = title_m.group(1) if title_m else ''
    match_name = re.sub(r'^Cricket commentary\s*\|\s*', '', raw_title).strip()

    # ── 2. Team names from match name ──────────────────────────────────────────
    teams_m = re.match(r'([^,]+?)\s+vs\s+([^,]+?),', match_name, re.IGNORECASE)
    team1 = teams_m.group(1).strip() if teams_m else 'Team A'
    team2 = teams_m.group(2).strip() if teams_m else 'Team B'

    # ── 3. Live score — pattern: "LSG 65/4 (8.5)" ─────────────────────────────
    abbrs = '|'.join(TEAM_ABBR.keys())
    score_m = re.search(rf'({abbrs})\s+(\d+)/(\d+)\s+\((\d+)\.(\d+)\)', text)
    score, wickets, overs_comp, balls_in = 0, 0, 0, 0
    batting_abbr = ''
    if score_m:
        batting_abbr = score_m.group(1)
        score       = int(score_m.group(2))
        wickets     = int(score_m.group(3))
        overs_comp  = int(score_m.group(4))
        balls_in    = int(score_m.group(5))

    # ── 4. Active batters — pattern: "(Mitchell Marsh 29(25) Nicholas Pooran 8(8))" ──
    meta_bat = re.search(r'\(([A-Z][^)]{3,60}\d+\(\d+\)[^)]*)\)', text)
    active_batters = []
    if meta_bat:
        bats = re.findall(r'([A-Z][a-zA-Z\s]+?)\s+(\d+)\((\d+)\)', meta_bat.group(1))
        for b in bats:
            balls = int(b[2])
            runs  = int(b[1])
            sr = round(runs / balls * 100, 1) if balls > 0 else 0.0
            active_batters.append({
                'name': b[0].strip(), 'runs': runs, 'balls': balls,
                'fours': 0, 'sixes': 0, 'sr': sr, 'dismissed': False
            })

    # ── 5. Batting / bowling from Next.js JSON chunks ─────────────────────────
    batsmen_map = {}
    bowlers_map = {}
    commentary  = []
    recent_balls = []
    fow_list    = []

    chunks = re.findall(r'self\.__next_f\.push\(\[1,"(.*?)"\]\)', text, re.DOTALL)
    for chunk in chunks:
        # Unescape unicode
        try:
            decoded = bytes(chunk, 'utf-8').decode('unicode_escape')
        except Exception:
            decoded = chunk

        # ── Batsman row: "batsmanName":"Mitchell Marsh","runs":29,"balls":25,... ──
        for bm in re.finditer(
            r'"batsmanName"\s*:\s*"([^"]+)"[^}]*?"runs"\s*:\s*(\d+)[^}]*?"balls"\s*:\s*(\d+)[^}]*?"fours"\s*:\s*(\d+)[^}]*?"sixes"\s*:\s*(\d+)',
            decoded
        ):
            name = bm.group(1)
            balls = int(bm.group(3))
            runs  = int(bm.group(2))
            batsmen_map[name] = {
                'name': name, 'runs': runs, 'balls': balls,
                'fours': int(bm.group(4)), 'sixes': int(bm.group(5)),
                'sr': round(runs / balls * 100, 1) if balls > 0 else 0.0,
                'dismissed': False,
            }

        # ── Bowler row ──
        for bw in re.finditer(
            r'"bowlerName"\s*:\s*"([^"]+)"[^}]*?"overs"\s*:\s*([\d.]+)[^}]*?"maidens"\s*:\s*(\d+)[^}]*?"runs"\s*:\s*(\d+)[^}]*?"wickets"\s*:\s*(\d+)[^}]*?"economy"\s*:\s*([\d.]+)',
            decoded
        ):
            name = bw.group(1)
            bowlers_map[name] = {
                'name': name, 'overs': float(bw.group(2)), 'maidens': int(bw.group(3)),
                'runs': int(bw.group(4)), 'wickets': int(bw.group(5)), 'economy': float(bw.group(6)),
            }

        # ── Commentary ──
        for cm in re.finditer(r'"commText"\s*:\s*"([^"]+)"[^}]*?"ballMetric"\s*:\s*([\d.]+)', decoded):
            comm_text = re.sub(r'<[^>]+>', '', cm.group(1))  # strip HTML tags
            ball_num  = float(cm.group(2))
            commentary.append({'text': comm_text, 'ball': ball_num})

            # Derive recent ball outcome
            tl = comm_text.lower()
            if 'six' in tl:                             recent_balls.append('6')
            elif 'four' in tl or 'boundary' in tl:      recent_balls.append('4')
            elif any(x in tl for x in ['wicket','out!!','caught','bowled','lbw','stumped','run out']): recent_balls.append('W')
            elif 'wide' in tl:                          recent_balls.append('wd')
            elif 'no ball' in tl:                       recent_balls.append('nb')
            elif 'no run' in tl:                        recent_balls.append('0')
            else:
                rm = re.search(r'(\d+) run', tl)
                recent_balls.append(rm.group(1) if rm else '1')

        # ── Fall of wickets ──
        for fw in re.finditer(
            r'"fowScore"\s*:\s*(\d+)[^}]*?"fowOver"\s*:\s*"([^"]+)"[^}]*?"fowBatsman"\s*:\s*"([^"]+)"',
            decoded
        ):
            fow_list.append({'score': int(fw.group(1)), 'over': fw.group(2), 'batsman': fw.group(3)})

    # ── 6. Resolve batters: prefer detailed data, fall back to meta ───────────
    batters = list(batsmen_map.values()) if batsmen_map else active_batters
    bowlers = list(bowlers_map.values())

    # ── 7. Teams ───────────────────────────────────────────────────────────────
    batting_full = TEAM_ABBR.get(batting_abbr, '')
    if batting_full:
        if any(t in team1 for t in [batting_full, batting_abbr]):
            batting_team, bowling_team = team1, team2
        else:
            batting_team, bowling_team = team2, team1
    else:
        batting_team, bowling_team = team1, team2

    # ── 8. Venue ───────────────────────────────────────────────────────────────
    city = 'Lucknow'
    for key, val in VENUE_CITY.items():
        if key in text.lower()[:3000]:
            city = val
            break

    # Deduplicate & sort commentary
    seen_balls = set()
    comm_dedup = []
    for c in sorted(commentary, key=lambda x: -x['ball']):
        if c['ball'] not in seen_balls:
            seen_balls.add(c['ball'])
            comm_dedup.append(c)

    all_players = [
        {'name': b['name'], 'runs': b['runs'], 'balls_faced': b['balls'],
         'sr': b.get('sr', 0), 'wickets': 0, 'catches': 0, 'team': batting_team}
        for b in batters
    ] + [
        {'name': bw['name'], 'runs': 0, 'balls_faced': 0, 'sr': 0,
         'wickets': bw['wickets'], 'catches': 0, 'economy': bw['economy'], 'team': bowling_team}
        for bw in bowlers
    ]

    return {
        'id': match_id,
        'name': match_name,
        'venue': city,
        'status': 'Live — IPL 2026',
        'batting_team': batting_team,
        'bowling_team': bowling_team,
        'target': None,
        'score': score,
        'overs_completed': overs_comp,
        'balls_in_over': balls_in,
        'wickets': wickets,
        'city': city,
        'batters': batters[:5],
        'bowlers': bowlers[:5],
        'recent_balls': list(reversed(recent_balls))[-12:],
        'fall_of_wickets': fow_list,
        'commentary': comm_dedup[:8],
        'all_players': all_players,
    }
